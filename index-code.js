    ...
    try {
      ...
      if (argoDomains.length > 0) {
        ...
        try {
          ...
          console.log(`${botName} is running`);
          await new Promise((resolve) => setTimeout(resolve, 3000));
          return await extractDomains(); // 递归后将获取到的域名交还给上层
        } catch (error) {
          console.error(`Error executing command: ${error}`);
        }
      }
...
// 主运行逻辑（总控核心：负责调用完全解耦的周边按需模块）
async function startserver() {
  try {
    // 【订阅节点及文件前置清理】
    if (typeof deleteNodes === 'function') deleteNodes();
    if (typeof cleanupOldFiles === 'function') cleanupOldFiles();

    // 【隧道二选一：固定隧道处理】
    if (ARGO_AUTH && ARGO_DOMAIN && typeof argoType === 'function') {
      argoType();
    }
    ...
    if (!finalDomain && typeof extractDomains === 'function') {
      finalDomain = await extractDomains();
    }
    ...
    // 【保活提交、订阅器提交、二进制文件执行后删除】
    if (typeof uploadNodes === 'function') await uploadNodes();
    if (typeof AddVisitTask === 'function') await AddVisitTask();
    if (typeof cleanFiles === 'function') cleanFiles();
